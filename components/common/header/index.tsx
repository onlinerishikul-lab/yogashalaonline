...
// Define the interface outside of the navigation items
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
                ...
                {activeDropdown === item.title && (
                  <div className="absolute bg-white shadow-lg top-full mt-2 rounded-md w-56 z-50">
                    {item.dropdown.map((subItem, index) => (
                      <div
                        key={subItem.title}
                        className={`relative border-b ${index === item.dropdown.length - 1 ? "border-none" : "border-gray-200"}`}
                      >
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            ...
                            {subItem.subDropdown.map((nestedItem, idx) => (
                              <Link
                                ...
                                className={`block px-4 py-2 text-sm text-black hover:bg-gray-100 border-b ${
                                  idx === subItem.subDropdown.length - 1 ? "border-none" : "border-gray-200"
                                }`}
                              >
                                {nestedItem.title}
                              </Link>
        ...
        {/* The rest of your header JSX goes here */}
      </div>
    </header>
  );
};
